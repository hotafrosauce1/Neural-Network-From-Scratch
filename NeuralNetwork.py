...
class Network:
    ...
    def predict(self, x):
        ...
        print("output_activation: {}, prediction: {}".format(output_activation, self.class_map[prediction_index]))
        return self.class_map[prediction_index]
    ...
    def update_weights(self, activations, errors, lr, batch_size):
        for layer_index in range(1, len(self.layers)):
            ...
            dC_dw = np.array([[activations[layer_index - 1][row] * errors[layer_index][col] for col in range(error_layer)]
...
n.train(X_train, y_train, batch_size = 1, epochs = 1000, lr = 0.01)
num_correct = 0
for i in range(len(X_test)):
    pred = n.predict(X_test[i])
